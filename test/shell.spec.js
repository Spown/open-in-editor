...
const shell = require('../lib/editors/common/shell');
  ...
  describe('platform ' + platform, function () {
    ...
    describe('shell command', function () {
      ...
      it('should take nested shell with arbitrary levels', function () {
        const res = shell.parse('sh -c', shell.parse('sh -c', shell.parse('echo foo')));
        assertEqual(res, {
      ...
      it('should substitute single param', function () {
        const res = shell.parse('sh -c {path}').withParams({
          path: 'foo'
        });
      ...
      it('should substitute param inside the argument', function () {
        const res = shell.parse('sh -c {path}:1').withParams({
          path: 'foo',
          line: 1
        });
      ...
      it('should propagate params to nested shells', function () {
        const res = shell.parse('sh -c', shell('{path}')).withParams({
          path: 'foo'
        });
        assertEqual(res, 'sh -c foo');
      });
      ...
      it('should propagete params having some spaces in value to nested shells', function () {
        const res = shell.parse('sh -c', shell.parse('echo {path}')).withParams({
          path: 'foo bar'
        });
        assertEqual(res, {
      ...
      it('should concat shell with params', function () {
        const res = shell('sh').concat(shell('{foo}').withParams({
          foo: 'bar'
        }));
      ...
      it('should handle path with spaces', function () {
        const res = shell('C:/Program Files (x86)/Microsoft VS Code/bin/code.cmd');
        assertEqual(res, {
